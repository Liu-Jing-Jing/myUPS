...
- (void)viewDidLoad
{
    ...
    for (UIView *child in self.stateControls)
    {
        if([child isKindOfClass:[UIButton class]])
        {
            ...
            [self.fourLightingButtons insertObject:btn atIndex:0];
        }
    }
...
- (NSMutableArray *)stateControls
{
    if(_stateControls == nil)
    {
        ...
        [_stateControls addObjectsFromArray:_fourStatusButton];
...
#warning 最后3个参数不确定什么意思
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if(startHistory == YES) [device historyMachineDataWithResultBlock:^(NSString * _Nonnull dataString) {
                    NSLog(@"准备同步历史数据");
                    //[MBProgressHUD showSuccess:@"正在同步历史数据"];
...
- (void)startParserHistoryData:(NSString *)string
{
    ...
    dispatch_once(&onceToken, ^{
        [MBProgressHUD showMessage:@"正在同步历史数据中, 请稍等便刻" toView:self.view];
    });
    if([[string lowercaseString] hasSuffix:@"feef010f41adfddf"] || [string rangeOfString:@"feef010f41adfddf"].location != NSNotFound)
    {
        // 开始解析历史数据  结束的标志位
        NSString *historyString = [string substringToIndex:[string rangeOfString:@"feef010f41adfddf"].location];
...
//
//        __block int syncCount = (int)uploadModels.count;
//
//        dispatch_semaphore_t semaphore = dispatch_semaphore_create(1);
//        dispatch_queue_t quene = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
//        for(int i=0;i<uploadModels.count;i++)
//        {
//            UpsModel *model = uploadModels[i];
//            __block BOOL isBreak = NO;
//            //任务1
//            dispatch_async(quene, ^{
//                dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
//                //请求A
//                model.uuids= @"0000000001";
////                [UpsNetworkingTools postUPSDataToServer:model success:^(NSArray * _Nonnull response)
////                 {
////                     NSLog(@"同步历史%d", i+1);
////                     syncCount--;
////                     if(syncCount==0)
...
////                         [MBProgressHUD showSuccess:@"同步已完成"];
////                     }
...
//            });
            [UpsNetworkingTools postManyUPSDataToServer:uploadModels success:^(id resultDict) {
                ...
                self.connectButton.hidden = NO;
                [MBProgressHUD hideHUDForView:self.view animated:YES];
                [MBProgressHUD showSuccess:resultDict[@"message"]];
            } failure:^(NSError * _Nonnull error) {
                [MBProgressHUD hideHUDForView:self.view animated:YES];
                [MBProgressHUD showError:@"同步出现错误,请在More里重新同步历史数据"];
                UnsyncUPSModel *unsynced = [UnsyncUPSModel new];
                unsynced.historyString = string;
                [unsynced save];
            }];
