...
- (IBAction)buttonClicked:(id)sender
{
    ...
    if(!self.lNameTF.text.length)
    {
        [message appendString:@"no last name\n"];
    ...
    else
    {
        ...
        if(errors.count >0)
        {
            [message appendString:@"Email Format Error\n"];
            btn.selected = YES;
        }
        else
        {
    ...
    dispatch_source_set_event_handler(timer, ^{
        ...
        if (timeout <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [weakSelf.getCodeButton setTitle:@"获取验证码" forState:UIControlStateNormal];
            });
        }else {
            
            //处于正在倒计时，在主线程中刷新button上的title，时间-1秒
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString * title = [NSString stringWithFormat:@"%d秒后获取验证码",timeout];
...
- (void)sendRegisterInfomation
{
    ...
    [[HttpTool shareInstance] registerUser:param1 withName:fullName Password:param2 Code:@"0000" Block:^(NSDictionary * _Nonnull dictionary) {
        // 注册新用户
        NSLog(@"%@", dictionary);
    }];
