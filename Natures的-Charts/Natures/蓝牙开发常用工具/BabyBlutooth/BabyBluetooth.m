...
- (NSArray *)retrieveConnectedPeripherals{
    if ([babySpeaker callback].babyOptions.discoverWithServices) {
        return [babyCentralManager->centralManager retrieveConnectedPeripheralsWithServices:[babySpeaker callback].babyOptions.discoverWithServices];
...
-(void)connectedPeripherals:(CBPeripheral*)peripheral{
    ...
    //清空pocjet
    babyCentralManager->pocket = [[NSMutableDictionary alloc]init];
    [babyCentralManager connectToPeripheral:peripheral];
}
