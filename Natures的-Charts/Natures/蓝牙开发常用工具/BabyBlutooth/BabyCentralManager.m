...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    switch (central.state) {
        ...
        case CBCentralManagerStatePoweredOff:
        {
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:BabyNotificationAtCentralManagerDisable object:@{@"central":central}];
        }
            break;
...
#warning 1
    ...
    //日志
    BabyLog(@"RSSI:%d强度-当扫搜索到了设备:%@", (int)RSSI, peripheral.name);
    [self addDiscoverPeripheral:peripheral];
    
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:BabyNotificationAtDidDiscoverPeripheral object:@{@"central":central,@"peripheral":peripheral,@"advertisementData":advertisementData,@"RSSI":RSSI}];
    //扫描到设备callback
