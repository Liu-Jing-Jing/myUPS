...
//      self.backgroundColor = self.colorPicker.currentColor;
        ...
        if (self.currentColorBlock) {
            self.currentColorBlock(self.colorPicker.R,self.colorPicker.G,self.colorPicker.B,self.colorPicker.L,self.colorPicker.tounchEnd);
        }
...
- (ColorPickerImageView *)colorPicker {
    if (!_colorPicker) {
        ColorPickerImageView *temp = [[ColorPickerImageView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        CGFloat w = self.frame.size.width;
        CGFloat h = self.frame.size.height;
...
-(void)layoutSubviews{
    ...
    _colorPicker.frame=CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    
    _slider.frame=CGRectMake(0, 0, 20, 20);
    CGFloat w = self.frame.size.width;
    CGFloat h = self.frame.size.height;
    self.slider.center = CGPointMake(w/2, h/2);
}
